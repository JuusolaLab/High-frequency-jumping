...
xdir = dot(repmat(xside,numpoints,1), intersectionpoints-mappos(1,:),2)/xlength*(mapsize(1)-1);
ydir = dot(repmat(yside,numpoints,1), intersectionpoints-mappos(1,:),2)/ylength*(mapsize(2)-1);
...
Xg =Xg(:);
Yg=Yg(:);
% radius =(mapsize(1)-1)/2;
% incorrect =(Xg-radius).^2+(Yg-radius).^2 > radius^2;
% Xg(incorrect) =[];
% Yg(incorrect) =[];
Map(sub2ind(size(Map), Yg+1, Xg+1))=Field(Xg,Yg);
Map(isnan(Map)) =0;